<template>
		<div id="table">
			<div class="add">
				<input type="text" v-model="addDetail.title" name="title" value="" placeholder="用户组名称" />
				...
				<tbody>
					<tr :key="index" v-for="(item,index) in authList">
						<td width="5%">{{index+1}}</td>
                         <td><router-link  :to="{ path: 'authUser', query: { authId: item.id}}" tag="div" class="user">{{item.title}}</router-link></td>
				...
				methods: {
					...
					update() {
						...
						for(let i = 0; i < _this.authList.length; i++) {
							if(_this.authList[i].id ==this.editid) {
								_this.authList[i] = {
